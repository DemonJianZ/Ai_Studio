...
const MIN_ZOOM = 0.1;
const MAX_ZOOM = 3;
const API_BASE = "http://192.168.20.30:8082";
          ...
          for (let b = 0; b < batchSize; b++) {
            try {
              ...
              if (procNode.data.mode === "text2img") {
                ...
                const resp = await fetch(`${API_BASE}/api/text2img`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    prompt: promptToUse,
                ...
                const resp = await fetch(`${API_BASE}/api/img2video`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                ...
                const resp = await fetch(`${API_BASE}/api/multi_image_generate`, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
      ...
      <header className="h-14 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-4 z-50 select-none shadow-md">
        ...
        <div className="text-[10px] text-slate-500 font-mono">
          ...
          <div className="flex rounded-md shadow-lg shadow-purple-900/40">
            <button onClick={handleRunClick} disabled={isRunning} className={`flex items-center gap-2 px-4 py-1.5 rounded-l-md font-bold text-sm transition-all min-w-[100px] justify-center ${isRunning ? "bg-slate-700 cursor-not-allowed" : "bg-purple-600 hover:bg-purple-500"}`}>
